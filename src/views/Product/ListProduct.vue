    ...
    <div class="px-16 py-12">
      <div class="flex justify-between">
        ...
        <div>
          ...
          <button class="text-sm px-4 py-2 focus:outline-none mr-5">
            Import
          </button>
          <button
            @click="$router.push('/product/add')"
            class="relative inline-flex items-center px-4 py-2 text-sm leading-5 font-medium rounded-md text-white bg-green-700 hover:bg-green-600 hover:text-gray-100 focus:outline-none focus:shadow-outline-green focus:border-green-300 active:bg-gray-100 active:text-gray-700 transition ease-in-out duration-150"
          >
            Tambah Produk
          </button>
        </div>
...
export default {
  ...
  async created() {
    ...
    try {
      const prodQuery = {
        p_page: this.page,
        p_limit: this.perPage,
        preload: ['belongs_to', 'product_image', 'product_detail'],
      };
      const prodRes = await AuthApiGet('/products', prodQuery);
      this.info = prodRes.data.data.elements;
    } catch (error) {
